...
function editModeAssets({ pc, factuurId, companyNaam, betaallink, pdfNogNietGemaakt = false, bijgewerktOp = null }) {
  // Het onderscheid eerste keer/opnieuw is voor de gebruiker niet relevant, de
  // knop heet altijd gewoon "Factuur opslaan". Alleen de meldingstekst erna
  // verschilt nog een klein beetje.
...
@media print{.drag-handle{display:none!important}.dblock:hover{outline:none!important}}`
  ...
  const button = `<button id="save-layout-btn">${saveLabel}</button>${betaallinkButton}<button id="save-default-btn">Maak dit de standaard voor ${companyNaam}</button>`
    ...
    betaallinkBtn.addEventListener('click', function(){
      ...
      fetch('/api/facturen/${factuurId}', {
        ...
        .then(function(r){
          ...
          // De PATCH maakt de PDF zelf al opnieuw, dus geen tweede ronde hier.
          var pdf = r.data && r.data.pdf
          if (pdf && !pdf.ok) throw new Error('betaallink opgeslagen, maar de PDF is niet bijgewerkt: ' + (pdf.fout || 'onbekende fout'))
 ...
 * @param {boolean} [opts.pdfNogNietGemaakt] editor-stand: er is nog geen PDF, de opslaanknop maakt hem voor het eerst
 * @param {string|null} [opts.bijgewerktOp] editor-stand: updated_at van de factuur, tegen opslaan over een nieuwere versie
 */
export function buildFactuurHtml(opts) {
  ...
  const edit = editMode ? editModeAssets({ pc, factuurId, companyNaam: cfg.naam, betaallink, pdfNogNietGemaakt, bijgewerktOp }) : null
