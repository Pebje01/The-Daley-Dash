...
export const GENERIC_COMPANY_CONFIG = {
  ...
  daleyphotography: {
    ...
    // Het fotografie-logo stond op de standaard 56px en oogde daardoor klein in
    // de kop. Op 84px is het ongeveer even hoog als het FACTUUR-blok ernaast.
    // Het logo is 3:1, dus de breedte moet mee omhoog, anders knijpt max-width
    // de hoogte alsnog terug.
    logoMaxHeight: 84,
    logoMaxWidth: 255,
...
function editModeAssets({ pc, factuurId, companyNaam, isConcept, factuurnummer, betaallink }) {
  const css = `
...
#maak-definitief-btn{background:#16a34a!important;color:#fff!important}`

  // Alleen bij een concept: eigen knop om het nummer uit de C-reeks in te
  // wisselen voor een echt factuurnummer. Bewust een aparte knop naast
  // "Opslaan als PDF" (dat blijft puur window.print(), zonder gevolgen) zodat
  // een concept nooit stilzwijgend een echt nummer claimt, alleen door hem te
  // bekijken of te printen.
  const definitiefButton = isConcept
    ? `<button id="maak-definitief-btn">Definitief maken</button>`
    : ''

  const betaallinkButton = `<button id="betaallink-btn">${betaallink ? 'Betaallink wijzigen' : 'Betaallink toevoegen'}</button>`

  const button = `${definitiefButton}<button id="save-layout-btn">Sla indeling op</button>${betaallinkButton}<button id="save-default-btn">Maak dit de standaard voor ${companyNaam}</button>`
...
(function(){
  ...
  var definitiefBtn = document.getElementById('maak-definitief-btn')
  if (definitiefBtn) {
    definitiefBtn.addEventListener('click', function(){
      var akkoord = confirm('Dit maakt van ${factuurnummer} een echte factuur met een nieuw nummer uit de bedrijfsreeks. De PDF verhuist naar de kwartaalmap en de gekoppelde uren worden afgeboekt. Dit kun je niet ongedaan maken. Doorgaan?')
      if (!akkoord) return
      definitiefBtn.disabled = true
      definitiefBtn.textContent = 'Bezig...'
      fetch('/api/facturen/${factuurId}/definitief', { method: 'POST' })
        .then(function(res){ return res.json().then(function(data){ return { ok: res.ok, data: data } }) })
        .then(function(r){
          if (r.ok && r.data && r.data.ok) {
            alert('Gelukt: ${factuurnummer} is nu ' + r.data.nummer + '.\\n\\nDe PDF is verplaatst naar de kwartaalmap en geopend, en de gekoppelde uren zijn afgeboekt. Dit tabblad toont zo weer verouderde gegevens, sluit het maar.')
            definitiefBtn.textContent = 'Gelukt: ' + r.data.nummer
          } else {
            definitiefBtn.disabled = false
            definitiefBtn.textContent = 'Definitief maken'
            alert('Definitief maken mislukt: ' + ((r.data && r.data.error) || 'onbekende fout'))
          }
        }).catch(function(err){
          definitiefBtn.disabled = false
          definitiefBtn.textContent = 'Definitief maken'
          alert('Definitief maken mislukt: ' + err)
        })
    })
  }

  var betaallinkBtn = document.getElementById('betaallink-btn')
  if (betaallinkBtn) {
    betaallinkBtn.addEventListener('click', function(){
      var huidige = ${JSON.stringify(betaallink || '')}
      var invoer = prompt('Betaallink (bijv. een Knab betaalverzoek of Mollie-checkout URL). Leeg laten en op OK drukken haalt de betaallink weg.', huidige)
      if (invoer === null) return // geannuleerd, niets doen
      var nieuweLink = invoer.trim() === '' ? null : invoer.trim()
      betaallinkBtn.disabled = true
      betaallinkBtn.textContent = 'Bezig...'
      fetch('/api/facturen/${factuurId}', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ molliePaymentUrl: nieuweLink }),
      }).then(function(res){ return res.json().then(function(data){ return { ok: res.ok, data: data } }) })
        .then(function(r){
          if (!r.ok) throw new Error((r.data && r.data.error) || 'onbekende fout')
          return fetch('/api/facturen/${factuurId}/regenerate-pdf', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: '{}' })
        })
        .then(function(res){ return res.json() })
        .then(function(data){
          if (data && data.ok) {
            alert(nieuweLink ? 'Betaallink opgeslagen en PDF bijgewerkt.' : 'Betaallink weggehaald en PDF bijgewerkt.')
        ...
        .catch(function(err){
          betaallinkBtn.disabled = false
          betaallinkBtn.textContent = huidige ? 'Betaallink wijzigen' : 'Betaallink toevoegen'
          alert('Opslaan mislukt: ' + err)
        })
    })
  }
...
export function buildFactuurHtml(opts) {
  ...
  // Logoformaat is per bedrijf instelbaar, want de logo's hebben een andere
  // verhouding en ogen op dezelfde hoogte niet even zwaar.
  const logoMaxHeight = cfg.logoMaxHeight ?? 56
  const logoMaxWidth = cfg.logoMaxWidth ?? 215
  ...
  // CONCEPT_PREFIX ('C') staat ook in lib/factuur-utils.ts, maar dat is TS en
  // dit bestand wordt ook los door node ingeladen (scripts/genereer-factuur.mjs),
  // dus hier bewust dezelfde simpele check als in FactuurDetailContent.tsx.
  const isConcept = String(factuurnummer).toUpperCase().startsWith('C-')
  const edit = editMode ? editModeAssets({ pc, factuurId, companyNaam: cfg.naam, isConcept, factuurnummer, betaallink }) : null
...
.logo{max-height:${logoMaxHeight}px;max-width:${logoMaxWidth}px;object-fit:contain;display:block;margin-top:1mm}
