...
export default function BtwAangiftePage({ params }: { params: { kwartaal: string } }) {
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        ...
        <div className="card border-red-200 bg-red-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <button onClick={fetchData} className="btn-secondary self-start sm:self-auto"><RefreshCw size={14} /> Opnieuw</button>
        </div>
      </div>
    )
  }
  ...
  return (
    // Vanaf xl (MacBook Pro 14") past de aangifte in één scherm: de root krijgt de
    // schermhoogte en de blokken staan in twee kolommen. Alleen de omzet- en
    // kostentabellen scrollen, binnen hun eigen kaart.
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1100px] xl:max-w-none xl:pt-8 xl:pb-4 xl:h-[calc(100dvh-var(--dash-topbar))] xl:overflow-hidden xl:flex xl:flex-col">
      {/* Header */}
      <div className="flex items-start justify-between mb-6 lg:mb-8 xl:mb-3 xl:shrink-0 gap-4 flex-wrap">
        <div>
      ...
      {/* Twee kolommen vanaf xl. Links de omzetkant (bank, omzet, correctie), rechts de
          kosten en de invulhulp. De volgorde in de DOM blijft gelijk aan die op kleinere
          schermen: bank, omzet, correctie, kosten, invulhulp. Links krijgt wat meer breedte,
          want de omzettabel heeft de meeste kolommen. */}
      <div className="xl:flex-1 xl:min-h-0 xl:grid xl:grid-cols-[1.2fr_1fr] xl:gap-4">
        <div className="xl:flex xl:flex-col xl:min-h-0 xl:gap-4">
          {/* Bank import */}
          <div className="card mb-6 xl:mb-0 xl:p-4 xl:shrink-0">
            <div className="flex items-center justify-between mb-3 xl:mb-2 flex-wrap gap-2">
          ...
          {/* Omzet. Vanaf xl vult deze kaart de resterende hoogte van de linkerkolom;
              de tabel en de controleblokken eronder scrollen samen binnen de kaart. */}
          <div className="card mb-6 xl:mb-0 xl:p-4 xl:flex-1 xl:min-h-0 xl:flex xl:flex-col">
            <h2 className="font-semibold text-body mb-1 xl:shrink-0">{isKas ? 'Betaalde omzet (kasbasis)' : 'Omzet: facturen vs bank'}</h2>
            <p className="text-caption text-brand-text-secondary mb-4 xl:mb-3 xl:shrink-0">
              {isKas
            ...
            <div className="overflow-x-auto">
              {/* Zes tot zeven kolommen: op smal scherm scrollt de tabel binnen de kaart */}
              {isKas ? (
                <table className="w-full min-w-[640px] xl:min-w-0">
              ...
              <div className="mt-5 rounded-brand-sm border border-brand-card-border bg-brand-page-light p-4">
                ...
                <div className="space-y-1">
                  {rec.facturenZonderOntvangst.map(f => (
                    <div key={f.id} className="flex items-center justify-between gap-3 text-caption">
              ...
              <div className="mt-5 rounded-brand-sm border border-amber-300 bg-amber-50 p-4">
                ...
                <div className="space-y-1.5">
                  {rec.ontvangstenZonderFactuur.map(t => (
                    <div key={t.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3 text-caption bg-white/70 rounded px-3 py-2">
        ...
        <div className="xl:flex xl:flex-col xl:min-h-0 xl:gap-4">
          ...
          <div className="card mb-6 xl:mb-0 xl:p-4 xl:shrink-0">
            ...
            <p className="text-caption text-brand-text-secondary mb-4 xl:mb-2">Neem deze bedragen over in je digitale aangifte.</p>
            <div className="overflow-x-auto">
      ...
      {toast && (
        <div className="fixed bottom-6 left-4 right-4 sm:left-auto sm:right-6 bg-brand-text-primary text-white text-caption px-4 py-2.5 rounded-brand-sm shadow-lg z-50">
          {toast}
        </div>
      )}
    </div>
...
function KpiCard({ icon, bg, label, waarde, sub, highlight }: {
  ...
  return (
    <div className={`card xl:p-4 ${highlight ? 'border-brand-lime-accent' : ''}`}>
...
function CorrectieBlok({ aangifte, onSave }: { aangifte: any; onSave: (v: Record<string, any>) => void }) {
  ...
  return (
    <div className="card mb-6 xl:mb-0 xl:p-4 xl:shrink-0">
      ...
      <p className="text-caption text-brand-text-secondary mb-4 xl:mb-3">
...
function KostenBlok({ kwartaal, kosten, onChange, setToast, berekening }: {
  ...
  return (
    // Vanaf xl vult deze kaart de resterende hoogte van de rechterkolom en scrolt
    // alleen de kostentabel, met vaste kop- en totaalregel.
    <div className="card mb-6 xl:mb-0 xl:p-4 xl:flex-1 xl:min-h-0 xl:flex xl:flex-col">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-1 xl:shrink-0">
        <div className="flex items-center gap-2">
      ...
      {open && (
        <div className="rounded-brand-sm border border-brand-card-border bg-brand-page-light p-3 mb-4 xl:mb-3 xl:shrink-0 grid grid-cols-2 sm:grid-cols-6 gap-2 items-end">
      ...
      {kosten.length === 0 ? (
        ...
        <div className="overflow-x-auto xl:flex-1 xl:min-h-0 xl:overflow-auto">
          <table className="w-full min-w-[560px] xl:min-w-0 text-caption">
            <thead className="xl:sticky xl:top-0 xl:z-10 xl:bg-brand-card-bg">
              <tr className="border-b border-brand-card-border text-brand-text-secondary">
