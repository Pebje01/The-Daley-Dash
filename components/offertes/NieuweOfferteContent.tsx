...
import { generateOffertePdf, saveOffertePdf } from '@/lib/pdf/offertePdf'
...
export interface NieuweOfferteContentProps {
  onClose?: () => void
...
export default function NieuweOfferteContent({
  onClose,
  onCreated,
  isDrawer = false,
  initialCompany: initialCompanyProp,
}: NieuweOfferteContentProps) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const { activeCompany } = useActiveCompany()
  const initialCompany = initialCompanyProp || (searchParams.get('bedrijf') as CompanyId) || activeCompany
  ...
  const [offerteDate, setOfferteDate] = useState(() => new Date().toISOString().split('T')[0])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Auto-save PDF (in map als ingesteld, anders browser download)
      const pdfCompany = getCompany(companyId)
      await saveOffertePdf(offerte, pdfCompany)

      if (onCreated) {
        onCreated(offerte.id)
      } else {
  ...
  return (
    <div className={isDrawer ? 'p-6 max-w-4xl' : 'max-w-4xl'}>
      {/* Header */}
      ...
      <div className="space-y-5">
        ...
        {mode === 'handmatig' && (
          <>
            ...
            {/* Datum & geldigheid */}
            <div className="card">
              <h2 className="font-semibold text-body mb-4">Datum & geldigheid</h2>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="label">Offertedatum</label>
                  <input className="input" type="date" value={offerteDate}
                    onChange={e => {
                      const newDate = e.target.value
                      setOfferteDate(newDate)
                      // Auto-update geldig-tot: +14 dagen
                      const d = new Date(newDate); d.setDate(d.getDate() + 14)
                      setValidUntilDate(d.toISOString().split('T')[0])
                    }} />
                </div>
                <div>
                  <label className="label">Geldig tot</label>
                  <input className="input" type="date" value={validUntilDate}
