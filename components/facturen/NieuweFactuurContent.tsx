...
interface NieuweFactuurContentProps {
  onClose?: () => void
...
export default function NieuweFactuurContent({
  onClose,
  onCreated,
  isDrawer = false,
  initialCompany: initialCompanyProp,
}: NieuweFactuurContentProps) {
  ...
  const [factuurDate, setFactuurDate] = useState(() => new Date().toISOString().split('T')[0])
  ...
  // Calculate due date display based on factuurDate
  const dueDate = new Date(new Date(factuurDate).getTime() + dueDays * 86400000)
  ...
  return (
    <div className={isDrawer ? 'p-6' : 'max-w-4xl'}>
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        {!isDrawer && (
          <button onClick={goBack} className="btn-secondary px-2.5"><ArrowLeft size={15} /></button>
        )}
        <div>
