...
- Gefilterd worden: dashboard (offerte- en factuurstats, abonnementen, open leads), offertes, facturen, betalingen plus kasstroom, abonnementen, klanten, uren, prospects en leads.
...
- **Het dashboard is de pagina voor vandaag**: wat moet ik doen en welk geld ligt klaar. Omzetcijfers horen daar niet; die staan op `/financieel` (sidebar: Financieel > Overzicht).
- Indeling (`app/(dashboard)/page.tsx`), vanaf `xl` een raster van 3 kolommen dat precies het scherm vult: links **Geld dat klaarligt** (uren te factureren, offertes te factureren, openstaand, te laat; incl. btw, open uren met 21% aangenomen), **Vandaag oppakken** (`components/dashboard/VandaagOppakken.tsx`: acties uit `/api/acties` plus leads waarvoor `moetVandaagOpgepakt()` geldt), **Pijplijn** (offertes verstuurd, leads in gesprek, open leads, MRR) en **Recente offertes en facturen** (4 elk). Rechts over de volle hoogte de to-do list. Vandaag oppakken is de rij die meerekt en zelf scrolt; op telefoon staat alles onder elkaar met de to-do list direct na het geld.
...
- `getUurKlanten()` geeft **standaard alleen de actieve klanten terug**; `?archief=1` op `/api/uren-klanten` haalt ze er allemaal bij. De urenpagina vraagt alles op en splitst zelf (voor de teller), `/klanten` toont ze allemaal met een Archief-badge, en de CRM-koppeling blijft op de standaard staan: daar wil je alleen actieve klanten kiezen.
