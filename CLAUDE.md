...
- **Detailkaart (september 2026):** 1320px breed, vaste hoogte (`h-[90dvh]`), drie kolommen vanaf `xl`: werk (AI-beoordeling, beschrijving) | details op een lichte lavendeltint (contactstatus, deadline, velden, uren, labels) | activiteit met opmerkingen. Koppelingen staan in de kop. Op `lg` schuift de activiteit onder de details, daaronder is het één scrollende kolom. Fase, "voor welk bedrijf" en de opvolgknoppen staan in de kop. Kopjes via `DetailSectie`, invoervelden via `VELD_INPUT` uit `lib/crm/stijl.ts`: altijd wit met een lichte rand, nooit de zware `.input` (kaartkleur met donkere rand). Vlakken zoals de AI-kwalificatie en de detailkolom zijn heel licht lila of grijs met een lichte rand; alles wat je invult is wit. Geldt ook voor het nieuw-recordformulier en de opvolgpopup (`OpvolgControls.tsx`). "Toegewezen aan" is weg: er is maar één gebruiker.
...
- **Twee plekken voor tekst, niet vier (21 september 2026): Beschrijving en Opmerkingen.**
  - **Beschrijving** (`raw.description`) is wat het is, en pas je aan. Wit tekstvlak, altijd zichtbaar, en hij **slaat zichzelf op zodra je uit het veld klikt** (`bewaarBeschrijving`, met "Opgeslagen" eronder); de knop Opslaan stuurt hem alleen nog als vangnet mee. Een wijziging logt de activiteit "Beschrijving bijgewerkt". Bij het omzetten van een lead naar een opdracht gaat de beschrijving mee (`promoteCrmRecord`); dat deed hij eerst niet. De lijstroute levert hem als `beschrijving` (`raw->>description`) voor de kolom Beschrijving.
  - **Details opdracht** (custom field uit de ClickUp-import) is opgegaan in Beschrijving: dat waren twee velden voor hetzelfde. `scripts/details-naar-beschrijving.mjs` (proef, of `--apply`, met reservekopie in `~/Library/Caches/daley-dash/`) zet de tekst over: leeg wordt overgenomen, verschillend komt eronder, al aanwezig wordt overgeslagen, en daarna gaat het veld leeg. Het veld verdwijnt uit de kaart zodra het leeg is (tot het samenvoegen blijft gevulde tekst zichtbaar) en gaat niet meer mee bij het omzetten.
  - **Opmerkingen** zijn losse, gedateerde notities in de kolom Activiteit: soort `opmerking` in `crm_activiteiten`, tekst in `nieuwe_waarde`, geen migratie nodig. Invoerveld bovenaan (Enter plaatst, shift+Enter nieuwe regel), elke opmerking een eigen wit blokje met aanpassen en weghalen, en een knop "Alleen opmerkingen". Route: POST/PATCH/DELETE op `/api/crm/activiteiten`; PATCH en DELETE werken alleen op opmerkingen, de rest van de feed is een logboek en blijft staan. De AI-kwalificatie leest ze mee als recente activiteit.
  - **Het grote Notities-veld (`raw.notes`) is weg.** Het werd nergens gebruikt (0 records). Staat er bij een oud record nog iets in, dan toont de kaart het alleen-lezen onder "Oude notities".
...
- Detail-modal: naam, status, beschrijving, opmerkingen, deadline, bewerkbare custom fields (dropdown/labels/datum/tekst/bedrag), relatiepaneel, uren-koppeling, promote (lead -> opdracht -> factuur)
...
- **Bovenaan het dashboard staat sinds 18 september 2026 weer de omzet** (`components/dashboard/OmzetBlok.tsx`): dit jaar, de gekozen maand met pijltjes, verwachte omzet, en een staafje per maand (klik kiest de maand). Daley vond het dashboard zonder omzet onduidelijk. De tabel per maand, betalingen en btw blijven op `/financieel` (sidebar: Financieel > Overzicht).
- **Herzien 21 september 2026 (Daley's keuze): het dashboard is een overzichtspagina, alles wat financieel is staat op `/financieel`.** Indeling vanaf `xl`, 3 kolommen: links **Omzet**, **Belangrijkste leads** (`components/dashboard/BelangrijksteLeads.tsx`, de rij die meerekt en zelf scrolt) en **Recente offertes en facturen**; rechts over de volle hoogte de to-do list.
- **Belangrijkste leads** verving "Vandaag oppakken": dat was een tweede to-do list naast de echte. Nu alleen lopende leads (geen Done/Closed, geen pauze of blokkade), gesorteerd op opvolging te laat, dan vandaag, dan waarde (`recordWaarde()` in `lib/crm/pipeline.ts`, veld "Prijs incl. btw"), dan AI-score. Hooguit 8 rijen.
- **De Pijplijn-rij is weg** van het dashboard. Openstaande offertes staan op Financieel onder "Ontvangen en uitstaand", MRR bij Abonnementen, lead-aantallen op het leadbord.
- **"Geld dat klaarligt" heet nu "Nog te factureren en te innen"** en staat op `/financieel` (`components/financieel/TeInnenBlok.tsx`): uren te factureren, offertes te factureren, openstaand, te laat (incl. btw, open uren met 21% aangenomen), met daaronder de acties die de Dash klaarzet (betaalherinneringen, offerte-follow-ups; `/api/acties`). Die acties stonden eerst in Vandaag oppakken.
- `/financieel` (`app/(dashboard)/financieel/page.tsx`): KPI's (omzet dit jaar, per maand, verwachte omzet, te laat), Nog te factureren en te innen, omzet per maand (tabel), verwachte omzet per maand, Ontvangen en uitstaand, BTW opzij (alleen onder TDE) en abonnementen.
- **Verwachte omzet** (`getFactuurStats`) = openstaande facturen + restant van akkoord-offertes + verstuurde offertes zonder factuur + open uren, alles ex. btw. Let op: de openstaande facturen zitten ook al in "Dit jaar".
...
- **Restant vervalt:** op de offerte (`components/offertes/OfferteFacturatie.tsx`, route `/api/offertes/[id]/facturatie`) zet je een restant dat nooit gefactureerd gaat worden op vervallen, bv. bij korting of een kleiner project. Kolommen `offertes.restant_vervallen_op` en `restant_vervallen_reden` (migratie `20260918_offerte_restant.sql`). **Vervallen kan alleen met een reden** (venster met voorzetjes via `melding.vraagTekst`, en server-side afgedwongen in de route); de reden staat daarna in het facturatieblok. Openzetten wist datum en reden. Zonder migratie rekent alles door alsof niets vervallen is, en geeft de knop een duidelijke fout.
